...
import data_utils
from data_utils import merge_dictionaries
...
def get_plot_data(data, key):
    ...
    switcher = {
        ...
        'rclcpp_interprocess_publish_mean': lambda data: [values['rclcpp_interprocess_publish_mean'] for _, values in sorted(data.items())],
        'rcl_publish_mean': lambda data: [values['rcl_publish_mean'] for _, values in sorted(data.items())],
        'rmw_publish_mean': lambda data: [values['rmw_publish_mean'] for _, values in sorted(data.items())],
        'dds_write_mean': lambda data: [values['dds_write_mean'] for _, values in sorted(data.items())],
        'sub_dds_on_data_mean': lambda data: [values['sub_dds_on_data_mean'] for _, values in sorted(data.items())],
        'sub_rcl_take_mean': lambda data: [values['sub_rcl_take_mean'] for _, values in sorted(data.items())],
        'sub_rclcpp_take_mean': lambda data: [values['sub_rclcpp_take_mean'] for _, values in sorted(data.items())],
        'sub_rmw_take_with_info_mean': lambda data: [values['sub_rmw_take_with_info_mean'] for _, values in sorted(data.items())],
        'sub_dds_take_mean': lambda data: [values['sub_dds_take_mean'] for _, values in sorted(data.items())],
        'sub_rclcpp_handle_mean': lambda data: [values['sub_rclcpp_handle_mean'] for _, values in sorted(data.items())],
        'total_mean': lambda data: [values['total_mean'] for _, values in sorted(data.items())]
    }
...
def plot_function(data_dict, x_key, y1_keys, y2_keys, separator, target = {}, savefig=False, fig=None, ax1=None):
...
def plot_profiling_over_subs(total_data, ax):
    for label, data in sorted(total_data.items()):
        x = get_plot_data(data, 'subs')

        # get separate profiling data
        rclcpp_interprocess_publish_mean = get_plot_data(data, 'rclcpp_interprocess_publish_mean')
        rcl_publish_mean = get_plot_data(data, 'rcl_publish_mean')
        rmw_publish_mean = get_plot_data(data, 'rmw_publish_mean')
        dds_write_mean = get_plot_data(data, 'dds_write_mean')
        sub_dds_on_data_mean = get_plot_data(data, 'sub_dds_on_data_mean')
        sub_rclcpp_take_mean = get_plot_data(data, 'sub_rclcpp_take_mean')
        sub_rcl_take_mean = get_plot_data(data, 'sub_rcl_take_mean')
        sub_rmw_take_with_info_mean = get_plot_data(data, 'sub_rmw_take_with_info_mean')
        sub_dds_take_mean = get_plot_data(data, 'sub_dds_take_mean')
        sub_rclcpp_handle_mean = get_plot_data(data, 'sub_rclcpp_handle_mean')
        total_mean = get_plot_data(data, 'total_mean')
        
        ax.plot(x, rclcpp_interprocess_publish_mean, 'r-')
        ax.plot(x, rcl_publish_mean, 'r-')
        ax.plot(x, rmw_publish_mean, 'r-')
        ax.plot(x, dds_write_mean, 'r-')
        ax.plot(x, sub_dds_on_data_mean, 'r-')
        ax.plot(x, sub_rclcpp_take_mean, 'r-')
        ax.plot(x, sub_rcl_take_mean, 'r-')
        ax.plot(x, sub_rmw_take_with_info_mean, 'r-')
        ax.plot(x, sub_dds_take_mean, 'r-')
        ax.plot(x, sub_rclcpp_handle_mean, 'r-')
        ax.plot(x, total_mean, 'r-')