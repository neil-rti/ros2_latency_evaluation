...
int performance_test::item_name_to_id(std::string name) {
    // if the name is in the format "node_7", "topic_1", "service_12" extract
    // the number from it otherwise create an hash code from the string

    // TODO: use only one regex with positive lookahead and start string
    // delimiter
    ...
    if (std::regex_match(name, e)) {
        ...
        std::regex_replace(std::ostream_iterator<char>(number_string),
                           name.begin(), name.end(), e2, "");
        // This will FAIL if the provided number starts with 0 e.g. "topic_01"
