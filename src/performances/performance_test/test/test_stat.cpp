...
TEST(StatTest, Stat_AddNegtiveSample) {
    performance_test::Stat<double> stat;
    stat.add_sample(-1);
...
TEST(StatTest, StatAddSamplesTest) {
    ...
    ASSERT_DOUBLE_EQ((double)s1, stat.samples()[0]);
    ...
    ASSERT_DOUBLE_EQ((double)s1, stat.max());
    ASSERT_DOUBLE_EQ((double)s2, stat.samples()[1]);
    ...
    ASSERT_DOUBLE_EQ((double)s2, stat.min());
    ASSERT_DOUBLE_EQ((double)s1, stat.max());
    ASSERT_DOUBLE_EQ((double)s3, stat.samples()[2]);
    ...
    ASSERT_DOUBLE_EQ((double)s1, stat.max());
    ASSERT_DOUBLE_EQ((double)s4, stat.samples()[3]);
}
