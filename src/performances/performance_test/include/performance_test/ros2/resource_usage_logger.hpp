...
class ResourceUsageLogger {
   public:
    ...
    void start(
        ...
        if (!_file.is_open()) {
            std::cout << "[ResourceUsageLogger]: Error. Could not open file "
                      << _filename << std::endl;
            std::cout << "[ResourceUsageLogger]: Not logging." << std::endl;
            return;
        }

        std::cout << "[ResourceUsageLogger]: Logging to " << _filename
                  << std::endl;
    ...
    void set_system_info(int pubs, int subs, float frequency) {
        if (_log == true) {
            std::cout
                << "[ResourceUsageLogger]: You have to set system info before "
                   "starting the logger!"
                << std::endl;
            return;
        }
   ...
   private:
    ...
    void _get() {
        ...
        _resources.elasped_ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(
                t2_real_start - _t1_real_start)
                .count();

        // Get CPU usage percentage
        auto t2_user = std::clock();
        auto t2_real = std::chrono::steady_clock::now();
        double time_elapsed_user_ms =
            1000.0 * (t2_user - _t1_user) / CLOCKS_PER_SEC;
        int n_threads = std::thread::hardware_concurrency();
        double time_elapsed_real_ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(t2_real -
                                                                  _t1_real)
                .count();
        _resources.cpu_usage =
            time_elapsed_user_ms / (time_elapsed_real_ms * n_threads) * 100;
        ...
        if (in.is_open()) {
            ...
            _resources.mem_virtual_KB =
                ((unsigned long int)std::stoi(virtual_mem_pages_string) *
                 _pagesize) >>
                10;
            in.close();
        } else {
    ...
    void _print_header() {
        ...
        _file << std::left << std::setw(wide_space) << std::setfill(separator)
              << "time[ms]";
        _file << std::left << std::setw(narrow_space) << std::setfill(separator)
              << "cpu[%]";
        _file << std::left << std::setw(wide_space) << std::setfill(separator)
              << "arena[KB]";
        _file << std::left << std::setw(wide_space) << std::setfill(separator)
        ...
        if (_got_system_info) {
            _file << std::left << std::setw(wide_space)
                  << std::setfill(separator) << "pubs";
            _file << std::left << std::setw(wide_space)
                  << std::setfill(separator) << "subs";
            _file << std::left << std::setw(wide_space)
                  << std::setfill(separator) << "frequency";
        }
    ...
    void _print() {
        ...
        _file << std::left << std::setw(wide_space) << std::setfill(separator)
              << std::setprecision(wide_space - 1)
              << std::round(_resources.elasped_ms);
        _file << std::left << std::setw(narrow_space) << std::setfill(separator)
        ...
        if (_got_system_info) {
            _file << std::left << std::setw(wide_space)
                  << std::setfill(separator) << _pubs;
            _file << std::left << std::setw(wide_space)
                  << std::setfill(separator) << _subs;
            _file << std::left << std::setw(wide_space)
                  << std::setfill(separator) << std::fixed << _frequency
                  << std::defaultfloat;
        }
