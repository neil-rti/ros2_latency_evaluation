...
class TemplateFactory {
   public:
    TemplateFactory(bool use_ipc = true, bool use_ros_params = true,
                    bool verbose_mode = false, std::string ros2_namespace = "")
        : _use_ipc(use_ipc),
     ...
     * where X, Y, etc, are all the numbers spanning from `start_id` to
     * `end_id`. The topic or service type is obtained parsing the msg_type
     * std::string
     */

    std::shared_ptr<Node> create_node(std::string name, bool use_ipc = true,
    ...
    std::vector<std::shared_ptr<Node>> create_periodic_publisher_nodes(
        ...
        msg_pass_by_t msg_pass_by, size_t msg_size = 0,
        rmw_qos_profile_t custom_qos_profile = rmw_qos_profile_default);
    ...
    std::vector<std::shared_ptr<Node>> create_server_nodes(
        int start_id, int end_id, std::string msg_type,
        rmw_qos_profile_t custom_qos_profile = rmw_qos_profile_default);

    /**
     * Helper functions that, given a node and a std::string describing the
     * msg_type, create the publisher/subscriber/client/server accordingly
     */

    void add_subscriber_from_strings(
        std::shared_ptr<Node> n, std::string msg_type,
    ...
    void add_periodic_publisher_from_strings(
        std::shared_ptr<Node> n, std::string msg_type, std::string topic_name,
        msg_pass_by_t msg_pass_by = PASS_BY_UNIQUE_PTR,
        rmw_qos_profile_t qos_profile = rmw_qos_profile_default,
        std::chrono::microseconds period = std::chrono::milliseconds(10),
        size_t msg_size = 0);

    void add_non_periodic_publisher_from_strings(
        std::shared_ptr<Node> n, std::string msg_type,
        std::string sub_topic_name, std::string pub_topic_name,
        msg_pass_by_t msg_pass_by = PASS_BY_UNIQUE_PTR,
        rmw_qos_profile_t qos_profile = rmw_qos_profile_default,
    ...
    void add_periodic_client_from_strings(
        std::shared_ptr<Node> n, std::string srv_type, std::string service_name,
        rmw_qos_profile_t qos_profile = rmw_qos_profile_default,
        std::chrono::microseconds period = std::chrono::milliseconds(10));
    ...
    std::vector<std::shared_ptr<Node>> parse_json_to_node_system(
        const nlohmann::json& topology_j,
        Tracker::TrackingOptions tracking_options);

    nlohmann::json read_json_file(const std::string& json_path);

    void change_data_processing_pipeline(nlohmann::json &node_system_j, const unsigned int number);

    void change_payload_size(const std::string& payload_size,
                             nlohmann::json& node_system_j);
    void change_qos_policy_value(const std::string& qos_policy,
                                 const std::string& qos_value,
                                 nlohmann::json& node_system_j);

    void change_publish_period_ms(const int change_publish_period_ms,
                                  nlohmann::json& node_system_j);

    std::vector<std::string> read_payloads_from_file(
   ...
   private:
    ...
    void create_node_entities_from_json(
        std::shared_ptr<Node> node, const nlohmann::json node_json,
        Tracker::TrackingOptions tracking_options = Tracker::TrackingOptions());

    void add_periodic_publisher_from_json(std::shared_ptr<Node> node,
                                          const nlohmann::json node_json);

    void add_non_periodic_publisher_from_json(std::shared_ptr<Node> node,
                                              const nlohmann::json node_json);

    void add_subscriber_from_json(std::shared_ptr<Node> node,
                                  const nlohmann::json node_json,
                                  Tracker::TrackingOptions tracking_options);

    void add_periodic_client_from_json(std::shared_ptr<Node> node,
                                       const nlohmann::json client_json);

    void add_server_from_json(std::shared_ptr<Node> node,
